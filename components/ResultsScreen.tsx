...
import { DiscScores, DiscType, HerrmannScores, HerrmannQuadrant, QuizType } from '../types';
...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ username, scores, analysis, isLoading, error, onRestart, quizType }) => {
  if (!scores || !quizType) {
    return (
      <div className="text-center p-8">
        <p className="text-slate-600 dark:text-slate-300">No results to display. Please take an assessment first.</p>
        <button onClick={onRestart} className="mt-4 px-6 py-2 bg-sky-600 text-white font-bold rounded-lg hover:bg-sky-700">
          Start Over
        </button>
      </div>
    );
  ...
  const dominantTrait = Object.keys(scores).reduce((a, b) => (scores as any)[a] > (scores as any)[b] ? a : b);
  ...
  return (
    <div className="bg-white dark:bg-slate-800 shadow-xl rounded-lg p-6 sm:p-8 animate-fade-in w-full">
      ...
      {dominantTrait && <p className="text-center text-xl text-slate-600 dark:text-slate-300 mb-8">Your primary trait appears to be <span className="font-bold" style={{color: (typeColors as any)[dominantTrait]}}>{dominantTrait}</span>.</p>}

      <div className="mb-8">
        <h2 className="text-2xl font-semibold text-slate-700 dark:text-slate-200 mb-4">Your Scores</h2>
        <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" strokeOpacity={0.2} />
                    <XAxis dataKey="name" tick={{ fill: 'currentColor' }} className="font-semibold text-slate-600 dark:text-slate-300" />
                    <YAxis tick={{ fill: 'currentColor' }} className="text-slate-600 dark:text-slate-300" />
                    <Tooltip
                    ...
                    <Bar dataKey="value">
                        {chartData.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={(typeColors as any)[entry.fullName]} />
                        ))}
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
      ...
      <div>
        <h2 className="text-2xl font-semibold text-slate-700 dark:text-slate-200 mb-4">Personalized Analysis for {username}</h2>
        {isLoading && (
